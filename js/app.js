 ...
 * - SMART PROMPT (VISUAL HAMMER): Atemwegs-Button zeigt nun als Task aktiv "DOKU FEHLT" an!
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (AppState.state === 'COMPRESSING') {
            ...
            if (AppState.airwayEstablished && window.CPR.AirwayTimer) {
                ...
                if (btnAw.dataset.isWarning !== "true") {
                    ...
                    `;
                    // WICHTIG: overflow-visible stellt sicher, dass das Ausrufezeichen sichtbar bleibt
    ...
    // =========================================================
    // 🌟 DEBUG GRID (Layout Tool - Aus app (21).js übernommen)
    // =========================================================
    ...
    if (resetBtn && resetBtn.parentNode) {
        ...
        function drawGrid() {
            ...
            if (topStats && !topStats.classList.contains('hidden')) topY = topStats.getBoundingClientRect().bottom;
        ...
        window.addEventListener('resize', () => { if (isDebugActive) drawGrid(); });

        const toggleBtn = document.createElement('button');
        toggleBtn.className = 'w-full bg-slate-100 text-slate-700 py-3 rounded-xl font-black text-sm border border-slate-300 active:scale-95 flex items-center justify-center gap-2 mb-4 transition-all';
